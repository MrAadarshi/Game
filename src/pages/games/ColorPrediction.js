...
const ColorPrediction = () => {
  ...
  const [prediction, setPrediction] = useState('green');
  ...
  // Color game specific states
  const [revealedColor, setRevealedColor] = useState(null);
  const colors = [
    { name: 'red', emoji: '🔴', multiplier: 2.0, chance: 50 },
    { name: 'green', emoji: '🟢', multiplier: 2.0, chance: 50 },
    { name: 'blue', emoji: '🔵', multiplier: 5.0, chance: 20 },
    { name: 'yellow', emoji: '🟡', multiplier: 10.0, chance: 10 }
  ];
  ...
  const [liveBets, setLiveBets] = useState([
    { user: 'Arjun_K294', amount: 50, multiplier: 10.0, win: 500, id: 1 },
    { user: 'Priya_S817', amount: 75, multiplier: 2.0, win: 150, id: 2 },
    { user: 'Rahul_P453', amount: 100, multiplier: 5.0, win: 500, id: 3 },
    { user: 'Ananya_M628', amount: 40, multiplier: 2.0, win: 80, id: 4 },
    { user: 'Vikram_R935', amount: 150, multiplier: 10.0, win: 1500, id: 5 }
  ]);
  ...
  const generateRandomBet = () => {
    ...
    const multipliers = [2.0, 2.0, 2.0, 5.0, 10.0]; // Weighted for realism
  ...
  useEffect(() => {
    const stored = localStorage.getItem('colorprediction_results');
    const storedStats = localStorage.getItem('colorprediction_stats');
  ...
  useEffect(() => {
    if (gameResults.length > 0) {
      localStorage.setItem('colorprediction_results', JSON.stringify(gameResults.slice(-50)));
    }
  }, [gameResults]);

  // Save game stats
  useEffect(() => {
    localStorage.setItem('colorprediction_stats', JSON.stringify(gameStats));
  }, [gameStats]);

  // Game logic
  const generateColorResult = () => {
    const rand = Math.random() * 100;
    
    if (rand < 10) return 'yellow'; // 10% chance
    if (rand < 30) return 'blue';   // 20% chance
    if (rand < 65) return 'red';    // 35% chance
    return 'green';                 // 35% chance
  };
  ...
  const handleBet = async (isAutomatic = false) => {
    ...
    try {
      await placeBet(betAmount, 'color-prediction', 'Color Prediction');
      setGameState('revealing');
      
      // Simulate color reveal animation
      setTimeout(() => {
        const resultColor = generateColorResult();
        const predictedColor = getColorData(prediction);
        const resultColorData = getColorData(resultColor);
        const isWin = prediction === resultColor;
        const winAmount = isWin ? betAmount * predictedColor.multiplier : 0;
        ...
        const gameResult = {
          ...
          prediction,
          resultColor,
          multiplier: predictedColor.multiplier,
          betAmount,
          winAmount,
          timestamp: new Date()
        };

        setRevealedColor(resultColor);
        ...
        processGameResult(betAmount, predictedColor.multiplier, 'color-prediction', 'Color Prediction', isWin);
    ...
    <div className="container animate-fadeInUp">
      ...
      <div style={{
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem', margin: 0, marginBottom: '1rem' }}>
          Predict the next color! Higher risk colors offer bigger rewards.
        </p>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{
              ...
              {colors.map(color => (
                  <button
                  key={color.name}
                  onClick={() => setPrediction(color.name)}
                  disabled={autoBetStats.isRunning}
                    style={{ 
                    padding: '1rem',
                    background: prediction === color.name ? 'var(--accent-green)' : 'var(--glass-bg)',
                    border: '1px solid var(--border-color)',
                    borderRadius: '12px',
                    color: prediction === color.name ? '#000' : 'var(--text-primary)',
                    fontSize: '0.85rem',
                    fontWeight: '600',
                    cursor: autoBetStats.isRunning ? 'not-allowed' : 'pointer',
                      transition: 'all 0.3s ease',
                ...
                >
                  <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{color.emoji}</div>
                  <div style={{ textTransform: 'capitalize', marginBottom: '0.2rem' }}>{color.name}</div>
                  <div style={{ fontSize: '0.75rem', opacity: 0.8 }}>
                    {color.multiplier}x • {color.chance}%
                  </div>
                  </button>
                ))}
              </div>
        ...
        <div className="game-area" style={{ textAlign: 'center' }}>
            ...
            {gameState === 'betting' && (
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)' }}>
                  Selected: {getColorData(prediction)?.emoji} {prediction.charAt(0).toUpperCase() + prediction.slice(1)}
                </p>
              </div>
            )}
            ...
            {gameState === 'revealing' && (
              ...
              <div style={{
                  fontSize: '6rem',
                  marginBottom: '2rem',
                  animation: 'colorSpin 1s infinite',
                ...
                <p style={{ color: 'var(--text-secondary)' }}>
                  Your prediction: {getColorData(prediction)?.emoji} {prediction.charAt(0).toUpperCase() + prediction.slice(1)}
                </p>
              </div>
            )}
            ...
            {gameState === 'result' && result && (
              ...
              <div style={{
                  ...
                  {getColorData(result.resultColor)?.emoji}
              </div>
              ...
              <div style={{
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                    Result: <strong style={{ color: 'var(--text-primary)' }}>
                      {result.resultColor.charAt(0).toUpperCase() + result.resultColor.slice(1)}
                    </strong>
              </div>
                  
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                    Your prediction: <strong>{prediction.charAt(0).toUpperCase() + prediction.slice(1)}</strong>
            </div>
          ...
          <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                  ...
                  <div>
                    <div style={{ color: 'var(--text-primary)', fontWeight: '600' }}>
                      {getColorData(game.resultColor)?.emoji || '?'} {game.resultColor?.charAt(0).toUpperCase() + game.resultColor?.slice(1) || 'Unknown'}
                    </div>
                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>
                      Predicted: {getColorData(game.prediction)?.emoji || '?'} {game.prediction?.charAt(0).toUpperCase() + game.prediction?.slice(1) || 'Unknown'} • ₹{game.betAmount || 0}
                    </div>
                  </div>
                </div>
            <div style={{ 
            ...
            {gameResults.length === 0 && (
              <div style={{
                ...
                No games played yet. Start predicting colors!
              </div>
            )}
          </div>
            </div>
        ...
        <div className="game-area">
          ...
          <div style={{
            ...
            <div style={{ color: 'var(--accent-green)', fontSize: '1rem', fontWeight: '700' }}>
              {formatNumber(liveBets.reduce((sum, bet) => sum + bet.win, 0) * 2.8)}
            </div>
          </div>
        </div>
      </div>
            ...
            <div style={{ color: 'var(--text-secondary)', lineHeight: 1.6, fontSize: '0.9rem' }}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem' }}>
                  <li>Choose a color you think will appear next</li>
                  <li>Place your bet and wait for the reveal</li>
                  <li>Win based on the rarity of your chosen color</li>
                </ul>
        </div>

              <div style={{ marginBottom: '1rem' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Color Odds:</strong>
                <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem' }}>
                  <li><strong>🔴 Red:</strong> 2x multiplier (35% chance)</li>
                  <li><strong>🟢 Green:</strong> 2x multiplier (35% chance)</li>
                  <li><strong>🔵 Blue:</strong> 5x multiplier (20% chance)</li>
                  <li><strong>🟡 Yellow:</strong> 10x multiplier (10% chance)</li>
                </ul>
      </div>
              ...
              <div>
                ...
                <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem' }}>
                  <li>Red/Green are safer with steady 2x returns</li>
                  <li>Blue offers good risk-reward balance at 5x</li>
                  <li>Yellow is highest risk but biggest reward at 10x</li>
                </ul>
              </div>
            </div>
          </div>
